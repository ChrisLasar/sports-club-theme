...
Sveltia CMS configuration for editing sports club content. Compatible with Decap CMS syntax.

```yaml
backend:
  name: git-gateway  # Or github/gitlab for direct Git integration
  branch: main

media_folder: "static/images/uploads"
public_folder: "/images/uploads"

# Optional: Use page bundles for team images
# Configure per-collection below
...
collections:
  - name: "teams"
    ...
    path: "{{slug}}/index"
    media_folder: ""  # Store images in team's page bundle
    public_folder: ""
    create: true
    slug: "{{slug}}"
    editor:
  ...
  - name: "events"
    ...
    fields:
      ...
      - {label: "Type", name: "type", widget: "select", options: ["fixture", "tournament", "training-camp", "social"]}
      ...
      - {label: "Status", name: "status", widget: "select", options: ["upcoming", "completed", "cancelled"], default: "upcoming"}
  ...
  - name: "posts"
    ...
    fields:
      ...
      - {label: "Social Image", name: "socialImage", widget: "image", required: false}
      - {label: "Discussion Link", name: "discussLink", widget: "string", required: false, hint: "Link to social media discussion (optional)"}
...
1. Place this file at `static/admin/config.yml` in your Hugo project
2. Add `static/admin/index.html` (Sveltia CMS entry point)
3. Configure Git backend (Git Gateway, GitHub, or GitLab)
4. Access CMS at `yoursite.com/admin/`

**Notes**:

- `media_folder` and `public_folder` control where uploaded images are stored
- Teams use page bundles (images stored with team's `index.md`)
- Privacy defaults enforced: `portraitConsent: false`, `contactPublic: false`
- All required image fields include alt text prompts (add `hint` for editors)
